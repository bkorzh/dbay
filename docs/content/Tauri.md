...
1. I had to remove the `compiled_frontend` base directory in `vite.config.ts`:
```ts
...
With this setup, the only thing that doesn't connect to the python server correctly is `bun run preview` (serving the built program directly from node). This is because that `meta.env.DEV` feature thing is no longer there or `true`, but the app is being served by something other than the python webserver. I've set it up so that any direct-browser access will go to the python webserver. 

For now, the frontend build copies the built files to `backend/backend/compiled_frontend`, which is where the static assets are served from with this python code in `main.py`:
